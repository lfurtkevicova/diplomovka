...
class GRASS:   
    def __init__(self):
        ...
        # cestaL z GUI
        self.gisdb = os.path.split(os.path.dirname(os.path.realpath(__file__)))[0]

        # SOFTVER
        grass7bin = self.getGrassPath()         
        # GRASS 7 a GISBASE
        #startcmd = [grass7bin]
        ...
        print out, err
        if p.returncode != 0:
            print >>sys.stderr, "ERROR: Cannot find GRASS GIS 7 start script (%s) %s" % (startcmd, err)
            sys.exit(-1)
...
class GUI(Tkinter.Frame):
    ...
    cestaL = ""
    recl1 = "recl1"
    #cesta je cesta k priecinku vypocet
    cesta = os.path.join(Gobj.gisdb,'vypocet')
    cesta_data = os.path.join(Gobj.gisdb,'data')
    cesta_nastroj = os.path.join(Gobj.gisdb,'nastroj')
 ...
 MIN  :   ?   :   ?\n      ?   :   ?   :   ?\n      ?   :   ?   :   ?\n             . . .     \n                       \n      ?   :   ?   :   ?\n      ?   :  MAX  :   ? ")
    ...
    def RG(self):
        try:
            self.Gobj.runGrass(self.nameL,self.nameM)
        except:
    ...
    def openL(self):
        ...
        #DEFAULT CESTA
        pr = askdirectory(initialdir=self.Gobj.gisdb)
    ...
    def openV(self):
        ...
        #DEFAULT CESTA
        priecinok = askdirectory(initialdir=self.cesta_data)
    ...
    def openI(self):          
        ...
        #DEFAULT CESTA
        priecinok = askdirectory(initialdir=self.cesta)
   ...
   deleted.\n\n   Do you really want to continue?")=="yes":
                    ...
                    nf_info = os.path.join(self.cestaI,'info') 
                    if not os.path.isdir(nf_info):
                        os.makedirs(nf_info)
                    nf_recl1 = os.path.join(self.cestaI,'recl1') #robim new folder
                    if not os.path.isdir(nf_recl1):
                        os.makedirs(nf_recl1)
                    nf_report = os.path.join(self.cestaI,'report') #robim new folder
                    if not os.path.isdir(nf_report):
                        os.makedirs(nf_report)
                    nf_recl2 = os.path.join(self.cestaI,'recl2') #robim new folder
                    if not os.path.isdir(nf_recl2):
                        os.makedirs(nf_recl2)
                        
                    # vytvorenie txt suborov na prvotnu reklasifikaciu
                    r1_G = os.path.join(self.cestaI,'recl1','recl1_G.txt') 
                    open(r1_G, 'w')
                    r1_DMR = os.path.join(self.cestaI,'recl1','recl1_DMR.txt') 
                    open(r1_DMR, 'w')
                    r1_S = os.path.join(self.cestaI,'recl1','recl1_S.txt') 
                    open(r1_S, 'w')
                    r1_E = os.path.join(self.cestaI,'recl1','recl1_E.txt') 
                    open(r1_E, 'w')
                    r1_DS = os.path.join(self.cestaI,'recl1','recl1_DS.txt') 
                    open(r1_DS, 'w')
                    r1_M = os.path.join(self.cestaI,'recl1','recl1_M.txt')  
                    open(r1_M, 'w')
                    r1_K = os.path.join(self.cestaI,'recl1','recl1_K.txt') 
                    open(r1_K, 'w')
                    r1_VK = os.path.join(self.cestaI,'recl1','recl1_VK.txt') 
                    open(r1_VK, 'w')
                    
                    # vytvorenie dalsich potrebnych txt suborov
                    open(os.path.join(self.cesta,'recl_y.txt'),"wb")
                    open(os.path.join(self.cesta,'recl_COV1.txt'),"wb")
                    open(os.path.join(self.cesta,'recl_COV2.txt'),"wb")
                    open(os.path.join(self.cesta,'recl_COV3.txt'),"wb")
    ...
    def open_file(self):
        ...
        options['initialdir'] = os.path.join(self.cesta,'info')
        options['parent'] = self.gui
        options['title'] = "Open a file"
        
        # z txt suboru INFO precita iba informacie o MIN a MAX hodnote bunky
        with tkFileDialog.askopenfile(mode='r', initialdir = os.path.join(self.cesta,'info')) as f_handle:
            pr = os.path.curdir
    ...
    def edit_savey(self):
        ...
        f = asksaveasfile(mode='w+', defaultextension=".txt", initialdir = self.cesta)
        if not f:
            return
    ...
    def open_filey(self):
        ...
        options['defaultextension'] = '.txt'
        options['filetypes'] = [('all files', '.*'), ('text files', '.txt')]
        options['initialdir'] = self.cesta
        options['parent'] = self.gui
        options['title'] = "Open a file"
        
        f_handle = os.path.join(self.cesta,'info_y.txt')
    ...
    def edit_save(self):
        ...
        options['parent'] = self.gui
        options['title'] = "Save as ..."
        f = asksaveasfile(mode='w+', defaultextension=".txt", initialdir = os.path.join(self.cesta,'recl1'))
        if not f:
            return
...
def main(): 
    ...
    o1 = PhotoImage(file=os.path.join(GUI.cesta_nastroj,'files','gui.gif'))
    def panelObr(o):   
